to setup
  ...
  reset-ticks
  cro num-of-turtles
  ask turtles [set shape one-of ["fish" "fish 2" "fish 3" "turtle"] set size 3 set heading 90 set color one-of [15 16 45 46 56 57 127 128] lineup]
  ask patches[
    ifelse pxcor > 27 and (pxcor + pycor) mod 2 = 0
      [set pcolor white]
    [set pcolor one-of [cyan 107 sky]]
    ]
end
...
to race
  ...
  ask turtles with [xcor >= max-pxcor - 3]
        [ask other turtles [die] set label "Victory" set label-color black]
end

to lineup
  set xcor (min-pxcor + 1)
  set ycor (((max-pycor * 2) + 1 )/ num-of-turtles * who - (max-pycor) + 1)
end
