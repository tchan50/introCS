...
to setup
  ca
  create-cats num-cats [set shape "cat" set size 2 set color brown setxy random-xcor random-ycor set lives 9]
  create-dogs num-dogs [set shape "dog" set size 2 set color grey setxy random-xcor random-ycor]
end

to wiggle
  every 0.1 [fd 1 lt random 30 rt random 30]
end

to go
  wiggle
  ask cats [if any? dogs-here [set lives (lives - 1) set label lives fd 1]]
  ask cats [if lives <= 0 [die]]


end

to-report numbercats
  report count cats
end
@#$#@#$#@
GRAPHICS-WINDOW
